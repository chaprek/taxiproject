      ...
      if(isset($arr['num_row']) && $id['phone'] != '381111111111'){ 
        ...
        if(time() - $arr['time'] < $config['timeout_orders']  && $id['phone'] != '380000000000'){
           ...
           if(in_array($city_id, $config['first_base'])){
            ...
            /*-≈сли прошло больше 5-ти минут возвращаем что нет машины-*/
                $all['orderId'] = "";
                ...
                if($id['phone'] != '380000000000'){
                    ...
                    if(in_array($city_id, $config['first_base'])){
                        ...
                        update_table('orders', array('meet' => 'ќтказ', 'endtask' => 6), array('num' => $order_id), $city_id);
                    } else {                    
                ...
                //вместо /home/user/data/www/site.ru/ указываем свой путь от корн€ сервера, куда должен писатьс€ лог
        ...
        /*-≈сли  заказа уже нет в новых заказа ищем его в таблице заказов-*/     
     } else {
